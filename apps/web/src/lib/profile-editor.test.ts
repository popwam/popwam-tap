...
import { moduleUpdateDecision, profileEditorMutationDecision, PROFILE_EDITOR_MODULE_KEYS } from "./profile-editor";
...
describe("Phase F profile editor contract", () => {
  ...
  it("keeps the editor registry code-owned and excludes future engines", () => {
    expect(PROFILE_EDITOR_MODULE_KEYS).toEqual(["IDENTITY", "ABOUT", "CONTACT", "SOCIAL", "LINKS", "SERVICES", "PORTFOLIO", "GALLERY", "BRANCHES"]);
    expect(PROFILE_EDITOR_MODULE_KEYS).not.toContain("BOOKING");
    expect(PROFILE_EDITOR_MODULE_KEYS).not.toContain("CATALOG");
