...
function pepper(){const value=process.env.OTP_PEPPER;if(!value)throw new Error("OTP_PEPPER_NOT_CONFIGURED");return value;}
...
export function hashRequestIp(ip:string){return createHmac("sha256",pepper()).update(`ip:${ip}`,"utf8").digest("hex");}
