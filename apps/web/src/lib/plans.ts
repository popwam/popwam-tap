...
export function mergeEntitlements<T extends Record<string, unknown>, U extends Record<string, unknown>>(plan: T, override?: U | null) {
  ...
  if (override) for (const key of [...LIMIT_KEYS, ...FEATURE_KEYS]) if (override[key] !== null && override[key] !== undefined) Object.assign(result, { [key]: override[key] });
  return result;
}
...
export async function getUserUsage(userId: string) {
  const [profiles, links, customFields, tags, uploads, storage] = await Promise.all([
    prisma.profile.count({ where: { userId } }), prisma.destination.count({ where: { userId } }),
    prisma.profileField.count({ where: { userId } }), prisma.tag.count({ where: { ownerId: userId } }),
  ...
  return { profiles, links, customFields, tags, uploads, storageBytes: storage._sum.sizeBytes || 0n };
}

export async function assertWithinLimit(userId: string, resource: "profiles" | "links" | "customFields" | "tags" | "uploads", increment = 1) {
  const [{ effective }, usage] = await Promise.all([getUserEntitlements(userId), getUserUsage(userId)]);
  const key = ({ profiles: "maxProfiles", links: "maxLinks", customFields: "maxCustomFields", tags: "maxTags", uploads: "maxUploads" } as const)[resource];
