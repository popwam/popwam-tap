...
export const draftProfileInclude = {
  ...
  publication: { select: { publishedRevision: { select: { revisionNumber: true, sourceDraftRevision: true, draftFingerprint: true } } } },
} satisfies Prisma.ProfileInclude;

export type DraftProfileData = Prisma.ProfileGetPayload<{ include: typeof draftProfileInclude }>;

export function managedProfileWhere(userId: string, profileId: string): Prisma.ProfileWhereInput {
  return {
    id: profileId,
    OR: [
      { userId },
...
export function profileDraftFingerprint(profile: DraftProfileData) {
  const scalar = {
    ...
    organizationNameAr: profile.organizationNameAr, organizationNameEn: profile.organizationNameEn,
    industryAr: profile.industryAr, industryEn: profile.industryEn,
    descriptionAr: profile.descriptionAr, descriptionEn: profile.descriptionEn,
    avatarUrl: profile.avatarUrl, coverUrl: profile.coverUrl, logoUrl: profile.logoUrl,
    phone: profile.phone, alternatePhone: profile.alternatePhone, whatsappBusiness: profile.whatsappBusiness,
    whatsappPrivate: profile.whatsappPrivate, email: profile.email, website: profile.website,
    facebook: profile.facebook, linkedin: profile.linkedin, github: profile.github, tiktok: profile.tiktok,
    locationText: profile.locationText, addressAr: profile.addressAr, addressEn: profile.addressEn,
    visibility: [profile.showAvatar, profile.showCover, profile.showDisplayName, profile.showTitle, profile.showBio, profile.showPhone, profile.showEmail, profile.showWebsite, profile.showLocation, profile.showWhatsappBusiness, profile.showWhatsappPrivate, profile.showSocialLinks, profile.showCustomFields, profile.showUploadedFiles, profile.showSaveContact],
    modules: profile.modules.map((item) => [item.moduleDefinition.key, item.enabled, item.visibility, item.sortOrder, item.configuration]),
    fields: profile.fields.map((item) => [item.id, item.label, item.labelAr, item.labelEn, item.value, item.type, item.isVisible, item.sortOrder]),
    destinations: profile.destinations.map((item) => [item.id, item.title, item.titleAr, item.titleEn, item.type, item.url, item.isActive, item.isVisible, item.sortOrder]),
    files: profile.uploads.map((item) => [item.id, item.publicUrl, item.isVisible, item.sortOrder]),
...
export function evaluateProfileReadiness(profile: DraftProfileData): ProfileReadiness {
  const issues: ReadinessIssue[] = [];
  const issue = (code: string, path: string, messageKey = `publishing.issue.${code.toLowerCase()}`) =>
  ...
  if (!profile.displayName.trim()) issue("DISPLAY_NAME_REQUIRED", "displayName");
  ...
  if (profile.category && profile.profileKind && profile.category.profileKind !== profile.profileKind) issue("CATEGORY_MISMATCH", "categoryId");
  if (profile.template && profile.profileKind && profile.template.profileKind && profile.template.profileKind !== profile.profileKind) issue("TEMPLATE_MISMATCH", "templateId");
  if (profile.template?.categoryId && profile.categoryId && profile.template.categoryId !== profile.categoryId) issue("TEMPLATE_MISMATCH", "templateId");
  ...
  if (!effectiveSlug) issue("SLUG_REQUIRED", "slug");
    ...
    if (!result.ok || result.slug !== effectiveSlug) issue(result.ok ? "SLUG_NOT_NORMALIZED" : result.error, "slug");
  }
  for (const media of profile.mediaAssets) {
    if (!["image/jpeg", "image/png", "image/webp"].includes(media.mimeType) || media.sizeBytes <= 0n || media.sizeBytes > 5n * 1024n * 1024n) {
  ...
  const enabled = new Set(profile.modules.filter((module) => module.enabled).map((module) => module.moduleDefinitionId));
  const identityModule = profile.modules.find((item) => item.moduleDefinition.key === "IDENTITY");
  if (!identityModule) {
    issues.push({ code: "REQUIRED_MODULE_MISSING", path: "modules.IDENTITY", module: "IDENTITY", messageKey: "publishing.issue.required_module_missing", severity: "ERROR", blocking: true });
  } else if (!identityModule.enabled) {
    issues.push({ code: "REQUIRED_MODULE_DISABLED", path: "modules.IDENTITY", module: "IDENTITY", messageKey: "publishing.issue.required_module_disabled", severity: "ERROR", blocking: true });
  } else if (identityModule.visibility !== "PUBLIC") {
    issues.push({ code: "REQUIRED_MODULE_NOT_PUBLIC", path: "modules.IDENTITY.visibility", module: "IDENTITY", messageKey: "publishing.issue.required_module_not_public", severity: "ERROR", blocking: true });
  }
  for (const rule of profile.template?.moduleRules ?? []) {
    if (rule.moduleDefinition.key === "IDENTITY") continue;
    if (!rule.required) continue;
    const instance = profile.modules.find((item) => item.moduleDefinitionId === rule.moduleDefinitionId);
    if (!instance) {
      issues.push({ code: "REQUIRED_MODULE_MISSING", path: `modules.${rule.moduleDefinition.key}`, module: rule.moduleDefinition.key, messageKey: "publishing.issue.required_module_missing", severity: "ERROR", blocking: true });
      continue;
    }
    ...
    if (instance?.visibility !== "PUBLIC") {
      issues.push({ code: "REQUIRED_MODULE_NOT_PUBLIC", path: `modules.${rule.moduleDefinition.key}.visibility`, module: rule.moduleDefinition.key, messageKey: "publishing.issue.required_module_not_public", severity: "ERROR", blocking: true });
      continue;
    }
    const complete = rule.moduleDefinition.key === "IDENTITY" ? Boolean(profile.displayName.trim())
      : rule.moduleDefinition.key === "CONTACT" ? Boolean(profile.phone || profile.email || profile.website || profile.whatsappBusiness || profile.whatsappPrivate)
        : rule.moduleDefinition.key === "LINKS" ? profile.destinations.some((item) => item.isActive && item.isVisible)
          : rule.moduleDefinition.key === "SERVICES" ? profile.services.some((item) => item.isVisible)
            : rule.moduleDefinition.key === "BRANCHES" ? profile.branches.some((item) => item.isVisible)
              : rule.moduleDefinition.key === "GALLERY" ? profile.uploads.some((item) => item.isVisible) || profile.mediaAssets.some((item) => item.state === "DRAFT_ATTACHED" || item.state === "PUBLISHED")
                : true;
    if (!complete) {
      issues.push({ code: "MODULE_INCOMPLETE", path: `modules.${rule.moduleDefinition.key}`, module: rule.moduleDefinition.key, messageKey: "publishing.issue.module_incomplete", severity: "ERROR", blocking: true });
    }
  }
  return {
...
export async function getOwnedDraft(userId: string, profileId: string) {
  return prisma.profile.findFirst({ where: managedProfileWhere(userId, profileId), include: draftProfileInclude });
}
...
export function revisionToProfileForAudience(profile: DraftProfileData, revision: PublishedRevisionData, audience: "PUBLIC" | "FRIEND") {
  ...
  return {
    ...
    organizationNameEn: identity && revision.showDisplayName ? revision.organizationNameEn : null,
    ...
    alternatePhone: contact && revision.showPhone ? revision.alternatePhone : null,
    email: contact && revision.showEmail ? revision.email : null,
    website: contact && revision.showWebsite ? revision.website : null,
    locationText: contact && revision.showLocation ? revision.locationText : null,
    ...
    contactNotesAr: null,
    contactNotesEn: null,
    whatsappBusiness: contact && revision.showWhatsappBusiness ? revision.whatsappBusiness : null,
    whatsappPrivate: contact && revision.showWhatsappPrivate ? revision.whatsappPrivate : null,
...
export async function publishProfile(userId: string, profileId: string, expectedDraftRevision: number) {
  const candidate = await getOwnedDraft(userId, profileId);
  if (!candidate) throw new Error("PROFILE_NOT_FOUND");
  if (candidate.draftRevision !== expectedDraftRevision) throw new Error("STALE_DRAFT");
  const initialReadiness = evaluateProfileReadiness(candidate);
  if (!initialReadiness.ready) return { ok: false as const, readiness: initialReadiness };
  return prisma.$transaction(async (tx) => {
    const profile = await tx.profile.findFirst({ where: managedProfileWhere(userId, profileId), include: draftProfileInclude });
    if (!profile) throw new Error("PROFILE_NOT_FOUND");
    if (profile.draftRevision !== expectedDraftRevision) throw new Error("STALE_DRAFT");
    ...
    const collision = await tx.profile.findFirst({ where: { slug: slug.slug, id: { not: profile.id } }, select: { id: true } });
    const historyCollision = await tx.profileSlugHistory.findUnique({ where: { slug: slug.slug }, select: { profileId: true } });
    if (collision || (historyCollision && historyCollision.profileId !== profile.id)) throw new Error("SLUG_TAKEN");
    const last = await tx.profileRevision.aggregate({ where: { profileId }, _max: { revisionNumber: true } });
    ...
    const mediaUrl = (mediaId: string) => `/api/public-profile-media/${mediaId}?revision=${revisionId}`;
    const publicMedia = (purpose: string) => {
      const asset = [...publicAssets].reverse().find((item) => item.purpose === purpose);
    ...
    const revision = await tx.profileRevision.create({
      data: {
        ...
        industryAr: profile.industryAr, industryEn: profile.industryEn,
        descriptionAr: profile.descriptionAr, descriptionEn: profile.descriptionEn,
        avatarUrl: publicMedia("AVATAR") || (profile.profileKind === "PERSONAL" ? onboardingImage : null) || profile.avatarUrl,
        coverUrl: publicMedia("COVER") || profile.coverUrl,
        logoUrl: publicMedia("LOGO") || (profile.profileKind === "BUSINESS" ? onboardingImage : null) || profile.logoUrl,
        phone: profile.phone, alternatePhone: profile.alternatePhone,
        whatsappBusiness: profile.whatsappBusiness, whatsappPrivate: profile.whatsappPrivate,
        email: profile.email, website: profile.website, facebook: profile.facebook, linkedin: profile.linkedin,
        github: profile.github, tiktok: profile.tiktok, vcfUrl: profile.vcfUrl,
        locationText: profile.locationText, addressAr: profile.addressAr, addressEn: profile.addressEn,
        contactNotesAr: profile.contactNotesAr, contactNotesEn: profile.contactNotesEn,
        theme: profile.theme, showAvatar: profile.showAvatar, showCover: profile.showCover,
        showDisplayName: profile.showDisplayName, showTitle: profile.showTitle, showBio: profile.showBio,
        showPhone: profile.showPhone, showEmail: profile.showEmail, showWebsite: profile.showWebsite,
        showLocation: profile.showLocation, showWhatsappBusiness: profile.showWhatsappBusiness,
        showWhatsappPrivate: profile.showWhatsappPrivate, showSocialLinks: profile.showSocialLinks,
        showCustomFields: profile.showCustomFields, showUploadedFiles: profile.showUploadedFiles,
        showSaveContact: profile.showSaveContact, allowInstallable: profile.allowInstallable,
        templateSlug: profile.template?.slug ?? profile.virtualCard?.template?.slug,
    ...
    if (profile.slug && profile.slug !== slug.slug) {
      await tx.profileSlugHistory.upsert({ where: { slug: profile.slug }, create: { slug: profile.slug, profileId }, update: {} });
      await tx.auditLog.create({ data: { actorId: userId, operation: "profile.slug.changed", targetId: profileId } });
    }
    if (publicAssets.length) await tx.profileMediaAsset.updateMany({ where: { id: { in: publicAssets.map((item) => item.id) } }, data: { state: "PUBLISHED" } });
    await tx.profileRevision.updateMany({ where: { profileId, id: { not: revision.id }, status: "PUBLISHED" }, data: { status: "SUPERSEDED" } });
    await tx.profilePublication.upsert({ where: { profileId }, create: { profileId, publishedRevisionId: revision.id }, update: { publishedRevisionId: revision.id } });
    await tx.profileMediaAsset.updateMany({ where: { profileId, state: "PUBLISHED", id: { notIn: publicAssets.map((item) => item.id) } }, data: { state: "DRAFT_ATTACHED" } });
    await tx.profile.update({ where: { id: profileId }, data: { lifecycle: "PUBLISHED", publishedAt: revision.publishedAt, isPublic: true, slug: slug.slug, draftSlug: slug.slug } });
    await tx.auditLog.create({ data: { actorId: userId, operation: "profile.published", targetId: profileId, metadata: { revisionNumber: revision.revisionNumber, profileKind: profile.profileKind, categoryKey: profile.category?.slug ?? null } } });
...
export async function transitionProfile(userId: string, profileId: string, action: "pause" | "resume" | "archive") {
  return prisma.$transaction(async (tx) => {
    const profile = await tx.profile.findFirst({ where: managedProfileWhere(userId, profileId), select: { lifecycle: true, isPrimary: true, publication: { select: { id: true } } } });
    if (!profile) throw new Error("PROFILE_NOT_FOUND");
    if (action === "archive") {
      if (profile.isPrimary) throw new Error("PRIMARY_PROFILE_ARCHIVE_FORBIDDEN");
      if (profile.lifecycle !== "DRAFT" && profile.lifecycle !== "PAUSED") throw new Error("PROFILE_TRANSITION_INVALID");
      await tx.profile.update({ where: { id: profileId }, data: { lifecycle: "ARCHIVED", archivedAt: new Date(), isPublic: false } });
      await tx.auditLog.create({ data: { actorId: userId, operation: "profile.archived", targetId: profileId } });
      return "ARCHIVED" as const;
    }
    if (!profile.publication) throw new Error("PUBLISHED_REVISION_REQUIRED");
    if (action === "pause" && profile.lifecycle !== "PUBLISHED") throw new Error("PROFILE_TRANSITION_INVALID");
    if (action === "resume" && profile.lifecycle !== "PAUSED") throw new Error("PROFILE_TRANSITION_INVALID");
    const lifecycle = action === "pause" ? "PAUSED" : "PUBLISHED";
    await tx.profile.update({ where: { id: profileId }, data: { lifecycle, isPublic: action === "resume" } });
    await tx.auditLog.create({ data: { actorId: userId, operation: action === "pause" ? "profile.paused" : "profile.resumed", targetId: profileId } });
    return lifecycle;
  });
}
