...
async function CardActivationInstructions({serialNumber,cardType,cardStatus,publicSlug}:{serialNumber:string;cardType:string;cardStatus:string;publicSlug:string}){return <main className="flex min-h-screen items-center justify-center px-5"><div className="card max-w-lg p-8 text-center"><p className="text-xs font-bold uppercase tracking-widest text-brand-400">POPWAM Tap</p><h1 className="mt-3 text-2xl font-black">بطاقة جاهزة للتفعيل / Ready to activate</h1><p className="mt-4 leading-7 text-slate-400">امسح رمز QR المنفصل المرفق بالمنتج لتفعيل البطاقة. لا يتغير هذا الرابط بعد التفعيل.<br/>Scan the separate activation QR supplied with this product. This permanent URL stays the same.</p><dl className="mt-6 grid grid-cols-3 gap-3 text-start"><div className="rounded-xl bg-white/5 p-3"><dt className="text-xs text-slate-500">Serial</dt><dd className="mt-1 font-mono text-xs" dir="ltr">{serialNumber}</dd></div><div className="rounded-xl bg-white/5 p-3"><dt className="text-xs text-slate-500">Type</dt><dd className="mt-1 font-mono text-xs" dir="ltr">{cardType}</dd></div><div className="rounded-xl bg-white/5 p-3"><dt className="text-xs text-slate-500">Status</dt><dd className="mt-1 font-mono text-xs" dir="ltr">{cardStatus}</dd></div></dl><Link href={`/activate/card/${publicSlug}`} className="btn-primary mt-6 w-full">تفعيل هذه البطاقة / Activate this card</Link></div></main>}
