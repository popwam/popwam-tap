...
export function NfcPlatformActions({ cardId, permanentUrl }: { cardId: string; permanentUrl: string }) {
  ...
  async function share() { if (navigator.share) await navigator.share({ title: "POP by POPWAM", url: permanentUrl }); else { await navigator.clipboard.writeText(permanentUrl); setStatus("URL copied."); } }
