...
export async function PublicStatus({ type }: { type: "notFound" | "paused" | "lost" | "disabled" | "fallback" | "unavailable" }) {
  ...
  const [Icon,title,description] = copy[type]; return <main className="flex min-h-screen items-center justify-center px-5"><div className="card max-w-md p-8 text-center"><div className="mx-auto flex size-14 items-center justify-center rounded-2xl bg-white/5"><Icon className="text-brand-400"/></div><h1 className="mt-5 text-2xl font-black">{title}</h1><p className="mt-3 leading-7 text-slate-400">{description}</p><Link href="/" className="btn-secondary mt-7">POPWAM Tap</Link></div></main>;
}
