...
export default async function UploadsPage(){const user=await requireUser();const [profile,files,{effective},usage]=await Promise.all([prisma.profile.findFirst({where:{userId:user.id,organizationId:null}}),prisma.uploadedFile.findMany({where:{uploaderUserId:user.id},orderBy:[{sortOrder:"asc"},{createdAt:"desc"}]}),getUserEntitlements(user.id),getUserUsage(user.id)]);if(!profile)return <p>Profile missing.</p>;return <><PageHeading eyebrow="Files" title="My Files" description="Upload, show or hide, and reorder safe public documents stored in R2."/><FileManager profileId={profile.id} allowed={effective.allowFileUploads} usageLabel={`${usage.uploads} / ${effective.maxUploads} files · ${Number(usage.storageBytes/1024n/1024n)} / ${Number(BigInt(effective.maxStorageBytes)/1024n/1024n)} MB`} initialFiles={files.map(file=>({...file,sizeBytes:file.sizeBytes.toString()}))}/></>;}
