...
import { parseProfileOpenPolicy, planOpenPolicyKey } from "@/lib/plan-open-policy";
...
export async function createProfile(data: FormData) {
  ...
  await prisma.$transaction(async tx => {
    ...
    const profile = await tx.profile.create({ data: { userId: user.id, type, profileKind:type==="ORGANIZATION"?"BUSINESS":"PERSONAL",lifecycle:"DRAFT",access:"PRIVATE",isPublic:false, displayName, displayNameAr: optional(data, "displayNameAr"), displayNameEn: optional(data, "displayNameEn"), organizationNameAr: type === "ORGANIZATION" ? optional(data, "displayNameAr") : null, organizationNameEn: type === "ORGANIZATION" ? optional(data, "displayNameEn") : null, primaryLanguage: text(data, "primaryLanguage") === "en" ? "en" : "ar", email: user.email } });
...
export async function saveAdminPlan(_previous:PlanActionState,data:FormData):Promise<PlanActionState>{
  ...
  let availableProfileTypes:Prisma.InputJsonValue;let availableThemes:Prisma.InputJsonValue;try{availableProfileTypes=JSON.parse(text(data,"availableProfileTypes"));availableThemes=JSON.parse(text(data,"availableThemes"));if(!Array.isArray(availableProfileTypes)||!availableProfileTypes.length||availableProfileTypes.some(value=>!["PERSONAL","PROFESSIONAL","CREATOR","BUSINESS"].includes(String(value)))||!Array.isArray(availableThemes)||!availableThemes.length||availableThemes.some(value=>!["CLASSIC_DARK","CLASSIC_LIGHT","ELEGANT_DARK","ELEGANT_LIGHT","BUSINESS_DARK","BUSINESS_LIGHT"].includes(String(value))))throw new Error();}catch{return{ok:false,code:"PLAN_INVALID_TEXT"};}
  const openPolicy=parseProfileOpenPolicy(text(data,"profileOpenMode")||"UNLIMITED",text(data,"profileOpenLimit"));if(!openPolicy)return{ok:false,code:"PLAN_INVALID_OPEN_POLICY"};
  const storefrontLimitMode=text(data,"storefrontLimitMode")||"UNLIMITED";const storefrontLimitRaw=text(data,"storefrontMaxItems");if(!["UNLIMITED","FIXED"].includes(storefrontLimitMode)||(storefrontLimitMode==="FIXED"&&!/^\d+$/.test(storefrontLimitRaw)))return{ok:false,code:"PLAN_INVALID_STOREFRONT"};const storefrontMaxItems=storefrontLimitMode==="UNLIMITED"?null:Number(storefrontLimitRaw);
  const customSlugAllowed=data.get("customSlugAllowed")==="on";const analyticsAllowed=data.get("analyticsAllowed")==="on";const maxCards=Number(values.maxCards);const maxFiles=Number(values.maxFiles);const allowThemes=data.get("allowThemes")==="on";const allowCustomTheme=data.get("allowCustomTheme")==="on";const allowBusinessCards=data.get("allowBusinessCards")==="on";
  const storefrontEnabled=data.get("storefrontEnabled")==="on";const storefrontProductsEnabled=data.get("storefrontProductsEnabled")==="on";const storefrontServicesEnabled=data.get("storefrontServicesEnabled")==="on";const storefrontWhatsappOrder=data.get("storefrontWhatsappOrder")==="on";const storefrontEmailOrder=data.get("storefrontEmailOrder")==="on";
  if((allowCustomTheme&&!allowThemes)||(Array.isArray(availableProfileTypes)&&availableProfileTypes.includes("BUSINESS")&&!allowBusinessCards))return{ok:false,code:"PLAN_FEATURE_CONFLICT"};
  if((storefrontEnabled&&(!allowThemes||!allowBusinessCards||!availableProfileTypes.includes("BUSINESS")||(!storefrontProductsEnabled&&!storefrontServicesEnabled)))||(!storefrontEnabled&&(storefrontProductsEnabled||storefrontServicesEnabled||storefrontWhatsappOrder||storefrontEmailOrder)))return{ok:false,code:"PLAN_INVALID_STOREFRONT"};
  const payload={name:nameEn,nameEn,nameAr,slug,description:optional(data,"descriptionEn"),descriptionEn:optional(data,"descriptionEn"),descriptionAr:optional(data,"descriptionAr"),isActive:data.get("isActive")==="on",sortOrder:Number(values.sortOrder),maxProfiles:Number(values.maxProfiles),maxVirtualCards:Number(values.maxVirtualCards),maxLinks:Number(values.maxLinks),maxCustomFields:Number(values.maxCustomFields),maxCards,maxTags:maxCards,maxFiles,maxUploads:maxFiles,maxStorageBytes,customSlugAllowed,allowCustomSlug:customSlugAllowed,allowThemes,allowCustomTheme,analyticsAllowed,allowAnalytics:analyticsAllowed,allowFileUploads:data.get("allowFileUploads")==="on",allowCustomIcons:data.get("allowCustomIcons")==="on",allowBusinessCards,allowWalletPasses:data.get("allowWalletPasses")==="on",allowCustomLinks:data.get("allowCustomLinks")==="on",allowInstallableProfiles:data.get("allowInstallableProfiles")==="on",storefrontEnabled,storefrontProductsEnabled,storefrontServicesEnabled,storefrontMaxItems,storefrontWhatsappOrder,storefrontEmailOrder,availableProfileTypes,availableThemes};
  try{const plan=id?await prisma.plan.update({where:{id},data:payload}):await prisma.plan.create({data:payload});await prisma.systemSetting.upsert({where:{key:planOpenPolicyKey(plan.id)},create:{key:planOpenPolicyKey(plan.id),value:openPolicy as Prisma.InputJsonValue},update:{value:openPolicy as Prisma.InputJsonValue}});await audit(admin.id,id?"admin.plan.update":"admin.plan.create",plan.id);revalidatePath("/admin/plans");revalidatePath(`/admin/plans/${plan.id}`);return{ok:true,id:plan.id};}catch(error){if(isUniqueConstraintError(error))return{ok:false,code:"PLAN_SLUG_IN_USE"};console.error("plan save failed",{operation:id?"plan.update":"plan.create",adminId:admin.id});return{ok:false,code:"PLAN_SAVE_FAILED"};}
}
