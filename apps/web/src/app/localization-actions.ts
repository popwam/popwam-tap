...
export async function saveLocalizationRuntime(data: FormData) {
  ...
  const addedCode=String(data.get("newLocaleCode")||"").trim().toLowerCase();
  const candidates=addedCode&&!current.locales.some(locale=>locale.code===addedCode)?[...current.locales,{code:addedCode,name:String(data.get("newLocaleName")||addedCode.toUpperCase()),nativeName:String(data.get("newLocaleNativeName")||data.get("newLocaleName")||addedCode.toUpperCase()),rtl:data.get("newLocaleRtl")==="on",enabled:false,published:false,displayOrder:current.locales.length,translations:{}}]:current.locales;
  const locales = candidates.map(locale => {
    ...
    return {
      ...locale,
      name: String(data.get(`name_${locale.code}`) || locale.name).trim().slice(0, 80) || locale.name,
      nativeName: String(data.get(`nativeName_${locale.code}`) || locale.nativeName).trim().slice(0, 80) || locale.nativeName,
      rtl: locale.code === "en" ? false : data.get(`rtl_${locale.code}`) === "on",
      enabled: locale.code === "en" || data.get(`enabled_${locale.code}`) === "on",
      published: locale.code === "en" || data.get(`published_${locale.code}`) === "on",
      displayOrder: Number(data.get(`order_${locale.code}`) || locale.displayOrder || 0),
      translations,
    };
  });
...
export async function saveTranslationKey(data: FormData) {
  ...
  if (!/^[a-z][a-z0-9]*(?:[._-][a-z0-9]+)*$/i.test(key)) throw new Error("TRANSLATION_KEY_INVALID");
  const english = String(data.get("value_en") || "").trim();
  if (!english) throw new Error("TRANSLATION_ENGLISH_REQUIRED");
  const current = await (async () => {
    const setting = await prisma.systemSetting.findUnique({ where: { key: LOCALIZATION_SETTING_KEY }, select: { value: true } });
  ...
  const next = sanitizeLocalizationConfig({ ...current, translationVersion: current.translationVersion + 1, locales });
  await prisma.$transaction([
    prisma.systemSetting.upsert({ where: { key: LOCALIZATION_SETTING_KEY }, create: { key: LOCALIZATION_SETTING_KEY, value: next as unknown as Prisma.InputJsonValue }, update: { value: next as unknown as Prisma.InputJsonValue } }),
    prisma.auditLog.create({ data: { actorId: admin.id, operation: "admin.localization.translation.save", metadata: { key, translationVersion: next.translationVersion } } }),
  ]);
  revalidatePath("/admin/localization");
