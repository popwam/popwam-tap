...
export async function POST(request:Request){
  ...
  const body=await request.json().catch(()=>null) as {assertion?:AuthenticationResponseJSON;deviceName?:unknown}|null;
  const result=await authenticateMobilePasskey(body?.assertion||null,typeof body?.deviceName==="string"?body.deviceName.slice(0,120):undefined,request.headers.get("x-pop-app-version")||undefined);
  if(!result)return Response.json({ok:false,error:"PASSKEY_AUTH_FAILED"},{status:400,headers:{"cache-control":"no-store"}});
  return Response.json(mobilePasskeySuccessResponse(result.session,result.user),{headers:{"cache-control":"no-store"}});
}
