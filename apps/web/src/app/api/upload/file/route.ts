...
export async function POST(request: Request) {
  ...
  try {
    ...
    try { const record = await prisma.$transaction(async tx => { const sortOrder = await tx.uploadedFile.count({ where: { profileId: profile.id } }); const created = await tx.uploadedFile.create({ data: { profileId: profile.id, uploaderUserId: user.id, storageKey: key, publicUrl: uploaded.url, originalFilename: file.name, mimeType: file.type, sizeBytes: file.size, title: String(data.get("title") || "").trim() || null, sortOrder } }); const linkOrder = await tx.destination.count({ where: { profileId: profile.id } }); await tx.destination.create({ data: { userId: user.id, profileId: profile.id, title: created.title || created.originalFilename, type: "FILE", url: created.publicUrl, iconKey: "file", sortOrder: linkOrder } }); return created; }); return Response.json({ ...record, sizeBytes: record.sizeBytes.toString() }); }
