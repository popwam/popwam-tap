...
export default async function InventoryPage() {
  ...
  return <>
    ...
    <div className="card overflow-x-auto"><table className="w-full min-w-[980px] text-sm"><thead><tr>{["SKU","Arabic name","English name","Type","Status","Physical quantity","Reserved","Available","Unit cost","Inventory value"].map(label => <th className="p-3 text-start" key={label}>{label}</th>)}</tr></thead><tbody>{items.map(item => { const physical = ledger.get(item.id) ?? 0; const available = Math.max(0, physical - item.quantityReserved); return <tr className={`border-t border-white/10 ${available <= item.reorderLevel ? "bg-amber-500/5" : ""}`} key={item.id}><td className="p-3 font-mono" dir="ltr">{item.sku}</td><td className="p-3">{item.nameAr}</td><td className="p-3">{item.nameEn}</td><td className="p-3">{item.type}</td><td className="p-3">{item.isActive?"ACTIVE":"INACTIVE"}</td><td className="p-3 font-bold">{physical}</td><td className="p-3">{item.quantityReserved}</td><td className="p-3 font-bold">{available}</td><td className="p-3">{String(item.unitCost)}</td><td className="p-3">{item.unitCost.mul(physical).toFixed(2)}</td></tr>; })}</tbody></table></div>
    <section className="card mt-6 p-5"><h2 className="font-bold">إضافة صنف / Add inventory item</h2><form action={createInventoryItem} className="mt-4 grid gap-3 sm:grid-cols-3"><input className="input font-mono" dir="ltr" name="sku" placeholder="SKU" required/><input className="input" name="nameAr" placeholder="الاسم بالعربية" required/><input className="input" name="nameEn" placeholder="English name" required/><select className="input" name="type">{Object.values(InventoryItemType).map(type => <option key={type}>{type}</option>)}</select>{[["quantityOnHand","Physical quantity"],["reorderLevel","Reorder level"],["unitCost","Unit cost"],["sellingPrice","Selling price"]].map(([name,placeholder]) => <input className="input" name={name} type="number" min="0" step={name.includes("Cost") || name.includes("Price") ? "0.01" : "1"} placeholder={placeholder} key={name}/>)}<input className="input sm:col-span-2" name="imageUrl" type="url" placeholder="Product image URL" dir="ltr"/><label className="flex items-center gap-2 rounded-xl border border-white/10 p-3"><input type="checkbox" name="isActive" value="on" defaultChecked/>Active product</label><button className="btn-primary sm:col-span-3">Save item</button></form></section>
  </>;
}
