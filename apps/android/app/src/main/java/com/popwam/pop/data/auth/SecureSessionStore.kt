...
class SecureSessionStore(private val context:Context):SessionStore {
    ...
    override suspend fun load():SessionTokens?{val blob=context.secureSessionDataStore.data.first()[key];current=blob?.let{runCatching{gson.fromJson(decrypt(it),SessionTokens::class.java)}.getOrNull()};return current}
    override suspend fun save(tokens:SessionTokens){context.secureSessionDataStore.edit{it[key]=encrypt(gson.toJson(tokens))};current=tokens}
    override suspend fun clear(){context.secureSessionDataStore.edit{it.remove(key)};current=null}
