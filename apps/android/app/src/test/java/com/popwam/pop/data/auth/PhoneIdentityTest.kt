...
class PhoneIdentityTest {
    ...
    @Test fun `whitespace and separators normalize through the same parser`(){assertEquals("+201001234567",PhoneIdentity.normalize("0100 123-4567","EG"))}
    @Test fun `typed failures do not expose parser exceptions`(){
        assertEquals(PhoneParseResult.Empty,PhoneIdentity.parse("","EG"))
        assertTrue(PhoneIdentity.parse("123","EG") is PhoneParseResult.Incomplete)
        assertTrue(PhoneIdentity.parse("12345678901234567890","EG") is PhoneParseResult.InvalidLength)
        assertEquals(PhoneParseResult.InvalidCountry,PhoneIdentity.parse("+999123456","ZZ"))
    }
